...
describe('Pull logs from CloudWatch', () => {
  ...
  let describeLogStreamsSpy = undefined;
  let describeLogStreamsStub = undefined;
  let getLogEventsSpy = undefined;
  ...
  beforeEach(() => {
    ...
    cloudwatchInterface = {
      describeLogStreams() { return { promise() { return new Promise((resolve, reject) => resolve(fakeLogStreams)); } }; },
      getLogEvents() { return { promise() { return new Promise((resolve, reject) => resolve(fakeLogEvents_streamA)); } }; }
    };
  ...
  it('pulls logs and formats them to a file correctly', async () => {
    ...
    const directoryContents = fs.readdirSync(path.join(context.logsRoot, context.projectInfo.variant));
    ...
    const logContents = fs.readFileSync(path.join(context.logsRoot, context.projectInfo.variant, directoryContents[0]), 'utf-8');
