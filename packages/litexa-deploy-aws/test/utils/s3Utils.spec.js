...
describe('S3Utils', () => {
  ...
  describe('#createAssetSets()', () => {
    ...
    it('should produce only one asset set with all assets inside if object properties is not defined', () => {
      ...
      const assetNames = [];
      for (let asset of Array.from(assetSets[0].assets)) {
      ...
      return expect(assetSets[0].params).to.equal(undefined);
    });
    ...
    it('should produce one asset set with all assets inside if only one object property is defined for all assets', () => {
      ...
      expect(assetNames).to.have.all.members([
        ...
        'image1.jpg', 'image2.jpg',
        'index.html', 'error.html']);
      return expect(assetSets[0].params.CacheControl).to.equal('max-age=3600');
    });
    ...
    it('should produce N asset sets if N object properties are defined that together cover all of the available assets', () => {
      ...
      let assetNames = [];
      for (asset of Array.from(assetSets[0].assets)) {
        assetNames.push(asset.name);
      }
      ...
      assetNames = [];
      for (asset of Array.from(assetSets[1].assets)) {
        assetNames.push(asset.name);
      }
      ...
      assetNames = [];
      for (asset of Array.from(assetSets[2].assets)) {
        assetNames.push(asset.name);
      }
      expect(assetNames).to.have.all.members(['style.css']);
      return expect(assetSets[2].params.CacheControl).to.equal('max-age=600');
    });
    ...
    it('should produce N asset sets if N-1 object properties are defined that together cover all but one of the available N asset sets', () => {
      ...
      expect(assetNames).to.have.all.members(['style.css']);
      return expect(assetSets[2].params).to.equal(undefined);
    });
    ...
    it('should correctly identify nested assets', () => {
      ...
      return expect(assetSets[0].params.CacheControl).to.equal('no-cache');
    });

    return it('should correctly apply default properties to non-filtered files wherever default properties are defined', () => {
      skillContext.deploymentOptions = {
        s3Configuration: {
          uploadParams: [
            {
      ...
      return expect(assetSets[2]).to.deep.equal(
        {
  ...
  return describe('#uploadAssetSet()', () => {
    const skillContext = { S3: { upload(params) { return { promise() {  } }; } } };
    const s3Context = {
      uploadIndex: 0,
      uploads: { length: 9 }
    };
    ...
    before(() => spy = spy(skillContext.S3, 'upload'));

    it('should not attempt to do any processing or uploading if asset set is undefined', () => {
      for (let assetSet of Array.from(assetSets)) {
        uploadAssetSet({ assetSet: undefined, s3Context, skillContext, logger, fs, mime });
      }

      expect(spy.callCount).to.equal(0);
      return expect(s3Context.uploadIndex).to.equal(0);
    });

    return it('should upload all of the assets across all of the asset sets', () => {
      for (let assetSet of Array.from(assetSets)) {
